function clockAngle(hh_mm:string) {

    // CASE#1 Empty input 
    if(hh_mm.length == 0) return "INPUT WAS EMPTY"
    // 
    if(!hh_mm.includes(":")) return "MISSING DELIMETER `:`"
    ...
    if(hourInt < 0 || minuteInt < 0) return "NEGATIVE NUMBER";
    // CASE#3 Over 24 hour and 59 minute format;
    if(hourInt > 24 || minuteInt > 59) return "INVALID INPUT `09:30`";
    // CASE#4 Not numerical
    if(!/^\d+$/.test(hour) || !/^\d+$/.test(minute)) return "INPUT IS NOT NUMBERIC"
